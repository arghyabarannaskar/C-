// eliminate duplicate combinations if there are duplicates elements in the array// 1 index cannot be considered more than once
#include<iostream>
#include<vector>
using namespace std;

void findCombination(int i, int amount, vector<int> &nums, vector<vector<int>> &ans, vector<int> &ds);

int main()
{
    int amount;
    vector<int> nums = {1, 1, 1, 1, 3, 5, 6, 7, 9};
    vector<vector<int>> ans;
    vector<int> ds;
    cout << "Enter the amount: ";
    cin >> amount;
    findCombination(0, amount, nums, ans, ds);
    cout << "Back from call";
    for(auto vect: ans){
        cout << endl;
        for(auto ele:vect){
            cout << ele << " ";
        }
    }

    return 0;
}

void findCombination(int ind, int target, vector<int> &nums, vector<vector<int>> &ans, vector<int> &ds){
    if(target == 0){
        ans.push_back(ds);
        return;
    }

    for (int i = ind; i < nums.size(); i++){
        if(i>ind && nums[i] == nums[i-1]) // in this index the valued element has been picked before so no need to include the same value again
            continue;
        if(nums[i]<=target){
            ds.push_back(nums[i]);
            findCombination(i + 1, target - nums[i], nums, ans, ds);
            ds.pop_back();
        }
    }
}